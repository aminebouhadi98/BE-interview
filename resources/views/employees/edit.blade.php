<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-white leading-tight">
            {{ __('Modifica Dipendente') }}
        </h2>
    </x-slot>

    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 mt-8">
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-100">Modifica Dipendente</h1>
                    ...
                    <select name="company_id" id="company_id"
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}"
                                {{ old('company_id', isset($employee) ? $employee->company_id : '') == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
